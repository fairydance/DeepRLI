...
from deeprli.datasets import LightDatasetForInfer
from deeprli.model import DeepRLI, DeepRLIForInterpretation

if __name__ == "__main__":
  parser = argparse.ArgumentParser(description="Infer DeepRLI (light)")
  ...
  parser.add_argument("--receptor-data-file", type=str, help="the path of a receptor data file relative to the root")
  parser.add_argument("--ligand-data-index", type=str, help="the path of an ligand index file relative to the root")
  parser.add_argument("--ligand-data-files", type=str, help="the path of ligand data files relative to the root")
  ...
  if config.get("model_format") is None: config["model_format"] = "model_object"
  ...
  data_loader = torch.utils.data.DataLoader(data, batch_size=config["batch"], collate_fn=LightDatasetForInfer.collate_fn)
  ...
  inference_results = pd.read_csv(os.path.join(config["data_root"], config["ligand_data_index"]))
  inference_results["scoring_score"] = p[0]
  inference_results["docking_score"] = p[1]
  inference_results["screening_score"] = p[2]
  result_file = os.path.join(config["save_path"], config["ligand_data_index"])
